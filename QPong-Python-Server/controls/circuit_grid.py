...
class CircuitGrid(pygame.sprite.RenderPlain):
    ...
    def __init__(self, xpos, ypos, circuit_grid_model, position):
        self.xpos = xpos
        self.ypos = ypos
        ...
        for row_idx in range(self.circuit_grid_model.max_wires):
            for col_idx in range(self.circuit_grid_model.max_columns):
                self.gate_tiles[row_idx][col_idx] = \
                    CircuitGridGate(circuit_grid_model, row_idx, col_idx, self.position)
            ...
            for row_idx in range(self.circuit_grid_model.max_wires):
                for col_idx in range(self.circuit_grid_model.max_columns):
                    self.gate_tiles[row_idx][col_idx].rect.centery = \
                        self.ypos + GRID_WIDTH * (col_idx + 1.35)
                    self.gate_tiles[row_idx][col_idx].rect.centerx = \
                        GRID_HEIGHT * QUBIT_NUM - (self.xpos + GRID_HEIGHT * (row_idx + 0.5))
        else:
            for row_idx in range(self.circuit_grid_model.max_wires):
                for col_idx in range(self.circuit_grid_model.max_columns):
                    self.gate_tiles[row_idx][col_idx].rect.centery = \
                        WINDOW_HEIGHT - (self.ypos + GRID_WIDTH * (col_idx + 1.35))
                    self.gate_tiles[row_idx][col_idx].rect.centerx = \
                        self.xpos + GRID_HEIGHT * (row_idx + 0.5)
    ...
    def highlight_selected_node(self, wire_num, column_num):
        ...
        if self.position == 'left':
            self.circuit_grid_cursor.rect.top = (self.ypos + GRID_WIDTH * (self.selected_column + 1)) \
                                               - round(0.375 * WIDTH_UNIT)
            self.circuit_grid_cursor.rect.left = GRID_HEIGHT * (QUBIT_NUM - 1) \
                                             - (self.xpos + GRID_HEIGHT * self.selected_wire) \
                                             + round(0.375 * WIDTH_UNIT)
        else:
            self.circuit_grid_cursor.rect.bottom = WINDOW_HEIGHT \
                                            - (self.ypos + GRID_WIDTH * (self.selected_column + 1)) \
                                               + round(0.375 * WIDTH_UNIT)
            self.circuit_grid_cursor.rect.left = self.xpos + GRID_HEIGHT * self.selected_wire \
                                             + round(0.375 * WIDTH_UNIT)
...
class CircuitGridGate(pygame.sprite.Sprite):
    ...
    def __init__(self, circuit_grid_model, wire_num, column_num, position):
    ...
    def update(self):
        ...
        if node_type == node_types.H:
            self.image, self.rect = load_image('gate_images/h_gate.png', -1, rotation_angle=self.gate_image_rotation)
