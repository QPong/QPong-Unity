...
class Input:
    ...
    def handle_input(self, level, screen, scene):
        ...
        if self.num_joysticks > 0:
            ...
            if gamepad_move:
                if joystick_hat == (-1, 0):
                    self.move_update_circuit_grid_display(circuit_grid[1], MOVE_LEFT)
                elif joystick_hat == (1, 0):
                    self.move_update_circuit_grid_display(circuit_grid[1], MOVE_RIGHT)
                elif joystick_hat == (0, 1):
                    self.move_update_circuit_grid_display(circuit_grid[1], MOVE_UP)
                elif joystick_hat == (0, -1):
                    self.move_update_circuit_grid_display(circuit_grid[1], MOVE_DOWN)
            self.gamepad_last_update = pygame.time.get_ticks()
        ...
        for event in pygame.event.get():
            ...
            if event.type == QUIT:
                ...
                if event.button == BTN_A:
                    # Place X gate
                    circuit_grid[1].handle_input_x()
                    circuit_grid[1].draw(screen)
                    self.update_paddle(level, screen, scene)
                    pygame.display.flip()
                elif event.button == BTN_X:
                    # Place Y gate
                    circuit_grid[1].handle_input_y()
                    circuit_grid[1].draw(screen)
                    self.update_paddle(level, screen, scene)
                    pygame.display.flip()
                elif event.button == BTN_B:
                    # Place Z gate
                    circuit_grid[1].handle_input_z()
                    circuit_grid[1].draw(screen)
                    self.update_paddle(level, screen, scene)
                    pygame.display.flip()
                elif event.button == BTN_Y:
                    # Place Hadamard gate
                    circuit_grid[1].handle_input_h()
                    circuit_grid[1].draw(screen)
                    self.update_paddle(level, screen, scene)
                    pygame.display.flip()
                elif event.button == BTN_RIGHT_TRIGGER:
                    # Delete gate
                    circuit_grid[1].handle_input_delete()
                    circuit_grid[1].draw(screen)
                    self.update_paddle(level, screen, scene)
                    pygame.display.flip()
                elif event.button == BTN_RIGHT_THUMB:
                    # Add or remove a control
                    circuit_grid[1].handle_input_ctrl()
                    circuit_grid[1].draw(screen)
                    self.update_paddle(level, screen, scene)
                    pygame.display.flip()
                ...
                if event.axis == AXIS_RIGHT_THUMB_X and joystick.get_axis(AXIS_RIGHT_THUMB_X) >= 0.95:
                    circuit_grid[1].handle_input_rotate(np.pi / 8)
                    circuit_grid[1].draw(screen)
                    self.update_paddle(level, screen, scene)
                    pygame.display.flip()
                if event.axis == AXIS_RIGHT_THUMB_X and joystick.get_axis(AXIS_RIGHT_THUMB_X) <= -0.95:
                ...
                if event.axis == AXIS_RIGHT_THUMB_Y and joystick.get_axis(AXIS_RIGHT_THUMB_Y) <= -0.95:
                    circuit_grid[1].handle_input_move_ctrl(MOVE_UP)
                    circuit_grid[1].draw(screen)
                    self.update_paddle(level, screen, scene)
                    pygame.display.flip()
                if event.axis == AXIS_RIGHT_THUMB_Y and joystick.get_axis(AXIS_RIGHT_THUMB_Y) >= 0.95:
                    circuit_grid[1].handle_input_move_ctrl(MOVE_DOWN)
                    circuit_grid[1].draw(screen)
                    self.update_paddle(level, screen, scene)
                    pygame.display.flip()
                ...
                # for player[0] on the left
                elif event.key == K_c:
                    circuit_grid[0].move_to_adjacent_node(MOVE_LEFT)
                    circuit_grid[0].draw(screen)
                    pygame.display.flip()
                elif event.key == K_b:
                    circuit_grid[0].move_to_adjacent_node(MOVE_RIGHT)
                    circuit_grid[0].draw(screen)
                    pygame.display.flip()
                elif event.key == K_f:
                    circuit_grid[0].move_to_adjacent_node(MOVE_UP)
                    circuit_grid[0].draw(screen)
                    pygame.display.flip()
                elif event.key == K_v:
                    circuit_grid[0].move_to_adjacent_node(MOVE_DOWN)
                    circuit_grid[0].draw(screen)
                    pygame.display.flip()
                elif event.key == K_1:
                    circuit_grid[0].handle_input_x()
                    circuit_grid[0].draw(screen)
                    self.update_paddle(level, screen, scene)
                    pygame.display.flip()
                elif event.key == K_2:
                    circuit_grid[0].handle_input_y()
                    circuit_grid[0].draw(screen)
                    self.update_paddle(level, screen, scene)
                    pygame.display.flip()
                elif event.key == K_3:
                    circuit_grid[0].handle_input_z()
                    circuit_grid[0].draw(screen)
                    self.update_paddle(level, screen, scene)
                    pygame.display.flip()
                elif event.key == K_4:con
                    circuit_grid[0].handle_input_h()
                    circuit_grid[0].draw(screen)
                    self.update_paddle(level, screen, scene)
                    pygame.display.flip()
                elif event.key == K_BACKSPACE:
                    circuit_grid[0].handle_input_delete()
                    circuit_grid[0].draw(screen)
                    self.update_paddle(level, screen, scene)
                    pygame.display.flip()

                # for player[1] on the right
                elif event.key == K_a:
                    circuit_grid[1].move_to_adjacent_node(MOVE_LEFT)
                    circuit_grid[1].draw(screen)
                    pygame.display.flip()
                elif event.key == K_d:
                    circuit_grid[1].move_to_adjacent_node(MOVE_RIGHT)
                    circuit_grid[1].draw(screen)
                    pygame.display.flip()
                elif event.key == K_w:
                    circuit_grid[1].move_to_adjacent_node(MOVE_UP)
                    circuit_grid[1].draw(screen)
                    pygame.display.flip()
                elif event.key == K_s:
                    circuit_grid[1].move_to_adjacent_node(MOVE_DOWN)
                    circuit_grid[1].draw(screen)
                    pygame.display.flip()
                elif event.key == K_x:
                    circuit_grid[1].handle_input_x()
    ...
    def update_paddle(self, level, screen, scene):
        ...
        statevector = level.statevector
        circuit_grid = level.circuit_grid
        statevector_grid = level.statevector_grid
        circuit = [circuit_grid_model[0].compute_circuit(), circuit_grid_model[1].compute_circuit()]

        for i in range(2):
            statevector_grid[i].paddle_before_measurement(circuit[i], scene.qubit_num, 100)
            statevector[i].arrange()
            circuit_grid[i].draw(screen)
